...
export const updateProfilePic=TryCatch(async(req:AuthenticatedRequest,res)=>{
    ...
    const uploadResult = data as { url: string; public_id: string };
    const [updatedUser] = await sql`
...
export const deleteSkillFromUser=TryCatch(
    async(req:AuthenticatedRequest,res)=>{
        ...
        RETURNING user_id`;
        if(result.length==0){
            throw new ErrorHandler(400,`Skill ${skillName.tim()} was not found`);
        }
        res.json({
...
export const applyForJob=TryCatch(async(req:AuthenticatedRequest,res)=>{
    ...
    if(user.role!=="jobseeker"){
        throw new ErrorHandler(403,"Forbidden: You are not allowed to apply for job");
    }
    ...
    const now=Date.now();
    // subscription logic when creating application
