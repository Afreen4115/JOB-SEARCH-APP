...
export const checkout = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const options = {
    amount: Number(PLAN_AMOUNT * 100), //collects in paise
    currency: "INR",
    notes: {
...
export const paymentVerification = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (isAuthentic) {
      const now = new Date();
      const thirstyDays = 30 * 24 * 60 * 60 * 1000; //30 days
      const expiryDate = new Date(now.getTime() + thirstyDays);
