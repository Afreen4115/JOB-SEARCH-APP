...
import { pubishToTopic } from "../utils/producer.js";
...
export const loginUser = TryCatch(async (req, res, next) => {
   ...
   if (!email || !password) {
    throw new ErrorHandler(400, "Please enter all details");
   }

   // Note: Fixed the typo "subscrption" to "subscription"
   const users = await sql`
   SELECT u.user_id, u.name, u.email, u.password, u.phone_number, u.role, u.bio,
   u.resume, u.profile_pic, u.subscription, 
...
export const forgotPassword = TryCatch(async (req, res, next) => {
    ...
    const successMessage = "If that email exists, we have sent a reset link";
    ...
    const message = {
        to: email,
        subject: "RESET Your Password - HireGenz",
        html: forgotPasswordTemplate(resetLink)
    };

    pubishToTopic("send-mail",message).catch((err)=>{
        console.log("message failed to sent",err)
    })
...
export const resetPassword=TryCatch(async(req,res,next)=>{
    ...
    //paina forgot password lo type set chestunam
