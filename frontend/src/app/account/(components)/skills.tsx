  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <Card className="shadow-lg border-2 overflow-hidden">
        ...
        {isYourAccount && (
          <>
            <div className="flex gap-3 flex-col sm:flex-row mx-8">
          ...
          {user.skills && user.skills.length > 0 ? (
            <div className="flex flex-wrap gap-3">
              {user.skills.map((e, i) => (
                <div
                  key={i}
                  className="group relative inline-flex items-center gap-2 border-2 rounded-full px-4 py-1.5
                     hover:shadow-sm transition-all duration-200 pl-4 pr-3 py-2"
                >
                  <span className="font-medium text-sm">{e}</span>
                  {isYourAccount && (
                    <button
                      onClick={() => removeSkillHandler(e)}
                      className="h-6 w-6 rounded-full text-red-500 flex items-center justify-evenly transition-all hover:bg-red-400 hover:scale-110 hover:text-white"
                    >
