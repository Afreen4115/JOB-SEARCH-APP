...
import { Briefcase, FileText, Filter, MapPin, Search, X } from "lucide-react";
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${job_service}/api/job/all?${params.toString()}`,
        {
          headers: {
        ...
        <div className="mb-8">
          ...
          {hasActiveFilter && (
            <div className="flex items-center gap-2 flex-wrap mb-4">
              ...
              {title && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-100 dar:bg-blue-900/30 text-blue-600 text-sm">
                  <Search size={14} />
                  {title}
                  <button
                    onClick={() => setTitle("")}
